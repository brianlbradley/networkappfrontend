...
//Setting routed:true makes sure all buttons are displayed
  const updatedSuggest = suggest.map(sugg => {
    if (sugg.id===id) {
       this.setState({
    suggest:suggest.filter(suggest => suggest.id===id),route:'suggestions'})
       this.setState({routed:true})
       this.setState({
            networkfilter:this.state.network.filter(netw=> netw.id===id),route:'suggestions'})
       this.setState({routed:true})
      ...
      this.state.route==='mynetwork' ? (
        <div>
         ...
         <Searchbox searchChange = {this.onSearchChange}
          ...
          <SearchSuggestions searchSuggestions = {this.onSuggestChange}
              onRouteChange = {this.onRouteChange}
              routed={this.state.routed}
